...
Use the included requirements file to install the necessary packages (including lifelines, pandas, seaborn, and scikit-learn).
...
🔬 Methodology
Data Preprocessing: Handled continuous clinical variables using StandardScaler and applied One-Hot Encoding for categorical features (avoiding the dummy variable trap). Transformed the censoring indicator into a clear "Event" format.

Exploratory Data Analysis (EDA): Visualized distributions of age, tumor size, and lymph node involvement, alongside a correlation matrix to check for multicollinearity.

Kaplan-Meier Estimation: Plotted baseline survival curves and compared treatment versus control groups to visually establish the prognostic advantage of Hormone Therapy.

Cox Proportional Hazards: Built a multivariate regression model to calculate Hazard Ratios for all clinical predictors.

Assumption Validation: Evaluated Schoenfeld residuals to guarantee the statistical integrity of the model's coefficients.
